...
3. **Partial distillation is allowed and normal.** If a spec owns only part of a
...
4. **Preserve information with no other home.** Cross-cutting notes the spec doesn't
   capture move to the relevant detailed section or a brief "Cross-cutting" note —
   never dropped.
5. Update the blueprint's index/table-of-contents entry for this section to say
   "distilled → spec <slug>" so the map and the sections agree.
6. **Idempotent.** If the section is already a digest+pointer for this spec, leave
